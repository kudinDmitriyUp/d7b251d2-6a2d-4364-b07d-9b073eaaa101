...
(function() {
  ...
  style.textContent = '' +
    ...
    '}' +
    'button.webild-selected,' +
    'a.webild-selected[href] {' +
    '  background-color: transparent !important;' +
    '}' +
  ...
  const showElementTypeLabel = (element, elementType) => {
    ...
    if (elementType === 'Image' || elementType === 'Video' || elementType === 'Button') {
      return;
    }
  ...
  const handleMouseOver = (e) => {
    ...
    const rawTarget = getMostSpecificElement(e.clientX, e.clientY, { preferMedia: !!e.altKey }) || e.target;
    const normalizedTarget = getIconContainerElement(rawTarget);
    const target = getTextContainerElement(normalizedTarget);
  ...
  const handleScroll = () => {
    ...
    scrollTimeout = setTimeout(() => {
      ...
      if (lastMouseX > 0 && lastMouseY > 0) {
        const target = getMostSpecificElement(lastMouseX, lastMouseY, { preferMedia: false });
